...
assert_equals({ (true, 2) }, { x = box(2); tas(x, { _ > 1 }, 3) });
assert_equals({ 3 }, { x = box(2); tas(x, { _ > 1 }, 3); *x });
assert_equals({ (false, 1) }, { x = box(1); tas(x, { _ > 1 }, 3) });
assert_equals({ 1 }, { x = box(1); tas(x, { _ > 1 }, 3); *x });

// tau = { b, p, f -> do({ o = own(b); (and(p(o), { put(b, f(o)); true }), o) }) }
assert_equals({ (true, 2) }, { x = box(2); tau(x, { _ > 1 }, inc) });
assert_equals({ 3 }, { x = box(2); tau(x, { _ > 1 }, inc); *x });
assert_equals({ (false, 1) }, { x = box(1); tau(x, { _ > 1 }, inc) });
assert_equals({ 1 }, { x = box(1); tau(x, { _ > 1 }, inc); *x });
...
// produce = { c, q, n, f -> nf = { l -> lt(list:size(l), n) }; while({ get(c) }, { mutate:awaits((c, q), { b, l -> or(not(b), { nf(l) }) }); loop:when(get(c), { v = f(); while({ and(get(c), { not(mutate:tau(q, nf, { __90_36 -> list:append($0_90_36, v) }).0) }) }, { () }); () }); () }); () }
// TODO
