...
public final class VariantConstraint implements Constraint
{
    ...
    {
        if (Session.isDebug())
            Session.debug(loc, "VariantConstraint: ({0}).satisfy({1})", dump(), type.dump());
        ...
        else if (otherOpts instanceof TypeApp)
        {
            ...
            if (otherBaseCons == Types.ASSOC)
            {
                ...
                final SubstMap valsSubst = assocVals.subsume(loc, opts.getValueTypes().subst(keySubst), env);
            ...
            if (Session.isDebug())
                Session.debug(loc, "type cons {0} is not handled, fail", otherBaseCons.dump());
        ...
        else
        {
            ...
            Session.error(loc, "internal error in ({0}).satisfy({1}): : unhandled argument {2} to Var TC ",
