...
public final class LambdaTerm extends AbstractScope implements Term {
  ...
  public LambdaTerm(
    ...
    this.signature =
        new Signature(loc, typeParamDecls, params, declaredResultType);

    this.capturedLambdaBindings = new LinkedHashMap<>();
    this.capturedModuleBindings = new LinkedHashMap<>();
  ...
  private void validateBody() {
    if (body.size() == 0) {
      ...
      if (lastStatement.isBinding()) Session
          .error(loc, "final statement in lambda is not a result expression");
    }
  }
   ...
   */
  Type getSignatureParamType(final int index) {
   ...
   */
  Type getParamType(final int index) {
  ...
  public void captureValueBinding(final ValueBinding binding) {
    ...
    final Map<String, ValueBinding> capturedBindingMap =
        scope.isLambda() ? capturedLambdaBindings : capturedModuleBindings;
    ...
    if (prev == null) {
      if (binding.isLet()) {
        ...
        if (!let.isIntrinsic() && let.getValue() == this) return;
      }

      capturedBindingMap.put(binding.getName(), binding);
    } else if (prev != binding) {
      Session.error(binding.getLoc(),
          "internal error: captured name \"{0}\" previously bound to value " +
              "{1}, new binding is {2}",
  ...
  public void addLet(final LetBinding let) {
    if (signature.getParams().containsKey(let.getName())) Session
        .error(let.getLoc(),
  ...
  public TypeBinding getLocalTypeBinding(final String name) {
    ...
    if (localTypeDef != null) return localTypeDef;
