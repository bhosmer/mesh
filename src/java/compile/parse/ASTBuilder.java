...
public class ASTBuilder {
  ...
  public static List<Statement> typeDefStmt(
      final Loc loc, final String name, final Type type
  ) {
    if (type.hasParams()) {
  ...
  public static List<Statement> importStmt(
      final Loc loc, final List<String> syms, final String module,
  ...
  public static List<Statement> typeAbsStmt(
      final Loc loc, final String name, final List<TypeParam> params,
      final Type body
  ) {
    if (params.isEmpty()) {
  ...
  public static List<Statement> letStmt(
      final Loc loc, final Term lval, final Type ltype, final Term rval
  ) {
  ...
  private static List<Statement> listDecompAssigns(
      final Term lhs, final Type type, final Term rhs
  ) {
  ...
  /**
   * Ensure a reference to a given expr (optionally explicitly typed).
   * If expr is not already a reference, or if we're asking that it
   * be annotated with a particular type, create a new temp
  ...
  private static RefTerm ensureRef(
      final Term expr, final Loc loc, final Type type,
      final List<Statement> results
  ) {
    // if expr is already a ref and we aren't demanding a particular type,
    // return existing ref.
    if (expr instanceof RefTerm && type == null) return (RefTerm) expr;

    // temp = expr
    final String tempName = "_" + loc.getLine() + "_" + loc.getColumn();
    results.add(new LetBinding(loc, tempName, type, expr));
  ...
  public static ParamBinding param(
    ...
    final ParamBinding param = new ParamBinding(loc, name, type);
  ...
  public static Term binaryExpr(
      final Term head, final List<Pair<Object, Term>> tail
  ) {
    return TermBinExprBuilder.build(head, tail);
  }

  /**
   * constructs a Verb structure, which wraps an infix operator (string or
   * Term),
   * plus left and right dimensionality ranks
   */
  public static Object verb(
      final int lefts, final Object op, final int rights
  ) {
   ...
   * unary expression
   */
  public static Term unaryExpr(
      final Loc loc, final String op, final Term term
  ) {
  ...
  public static Term applyChain(
      final Term base, final List<Pair<Term, ApplyFlavor>> chain
  ) {
   ...
   */
  public static RefTerm inlineParam(
      final Loc loc, final String prefix, final String strpos
  ) {
  ...
  private static SimpleLiteralTerm numLiteral(
      final Loc loc, String text, final int radix
  ) {
  ...
  public static MapTerm mapLiteral(
      final Loc loc, final List<Pair<Term, Term>> pairs
  ) {
  ...
  public static TupleTerm tupLiteral(
      final Loc loc, final Term head, final List<Term> tail
  ) {
  ...
  public static LambdaTerm lambda(
      final Loc loc, final List<TypeParam> typeParams,
      final List<ParamBinding> params, final Type returnType,
      final List<Statement> body
  ) {
    /*
    ...
    TODO will need wildcards for any undeclared params. Do we have?
    TODO also need to reconcile exact/inexact typing differences

    if (params != null) {
      final List<Term> items = new ArrayList<>();
      final List<Type> types = new ArrayList<>();
      for (final ParamBinding param : params) {
        items.add(new RefTerm(param.getLoc(), param.getName()));
        types.add()
      }

      final TupleTerm lval = new TupleTerm(loc, items);
  ...
  public static Type quantifiedType(
      final Loc loc, final List<TypeParam> params, final Type type
  ) {
    type.setLoc(loc);
  ...
  public static Type binaryTypeExpr(
      final Type left, final List<Pair<Object, Type>> rights
  ) {
  ...
  public static Type unaryTypeExpr(
      final Loc loc, final String op, final Type arg
  ) {
  ...
  public static Type typeApplyChain(
      final Loc loc, final Type base, final List<Type> chain
  ) {
    Type result = base;
  ...
  public static Type mapType(
      final Loc loc, final Type keyType, final Type valueType
  ) {
  ...
  public static Type tupType(
      final Loc loc, final Type head, final List<Type> tail
  ) {
    return Types
   ...
   *
   */
  public static Type recType(
      final Loc loc, final List<Pair<Term, Type>> pairs
  ) {
  ...
  public static TypeParam typeParam(
      final Loc loc, final String name, final Kind kind
  ) {
  ...
  public static Kind arrowKind(
      final Loc loc, final Kind head, final List<Kind> tail
  ) {
  ...
  public static List<String> distribute(
      final String prefix, final List<String> rest
  ) {
