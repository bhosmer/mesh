...
public final class TypeChecker extends ModuleVisitor<Type> implements TypeEnv
{
  ...
  @Override protected void processScope(final Scope scope)
  {
    if (Session.isDebug())
   ...
   */
  @Override protected void processStatement(final Statement statement)
  {
  ...
  private void initLet(final LetBinding let)
  {
    ...
    if (let.hasDeclaredType())
    {
      ...
      if (Session.isDebug())
        Session.debug(let.getLoc(), "seeded {0} : {1} (decl)", let.dump(),
            let.getType().dump());
    }
    else
    {
      ...
      if (letType != null)
      {
        ...
        if (Session.isDebug())
          Session.debug(let.getLoc(), "seeded {0} : {1} (prior)", let.dump(),
              let.getType().dump());
      }
      else
      {
        ...
        if (Session.isDebug())
          Session
              .debug(let.getLoc(), "seeded {0} : {1}", let.dump(), type.dump());
      }
    }
  ...
  private void finishGroup()
  {
    ...
    if (Session.isDebug())
      Session.debug("{0}** finishing group, ambients = {1}", indent(),
          TypeDumper.dumpList(ambientVars));
    ...
    if (!deferredDefs.isEmpty())
    {
      ...
      {
        final SubstMap newParams = finishDef(deferredDef, ambientVars, params);
   ...
   * quantified.
   */
  private SubstMap finishItem(final Typed item, final Set<TypeVar> ambientVars,
    ...
    if (quantItemType.hasVars() || (hasDeclType && quantDeclType.hasVars()))
    {
      ...
      if (Session.isDebug())
        Session.debug(item.getLoc(), "{0} {1} : {2}{3}",
            ambientParams.isEmpty() ? "deferring" : "re-deferring", item.dump(),
            quantItemType.dump(),
    ...
    else
    {
      ...
      if (hasDeclType)
      {
        ...
        else
        {
          ...
          Session.error(quantDeclType.getLoc(),
              ...
              declaredDump, inferredDump, declaredDump.equals(inferredDump) ?
                  "from " + quantItemType.getLoc() : "");
        }
      ...
      if (Session.isDebug())
        Session.debug(item.getLoc(), "{0} {1} : {2}{3}",
            ...
            item.dump(), item.getType().dump(),
            hasDeclType ? " decl " + quantDeclType.dump() : "");
    }

    // return all generated params
    return hasDeclType ? declTypeParams.compose(item.getLoc(), itemTypeParams) :
        itemTypeParams;
  }
   ...
   */
  private SubstMap buildTypeParams(final Type type,
      final Set<TypeVar> ambientVars, final SubstMap ambientParams)
  {
    ...
    final Set<TypeVar> newVars =
        Sets.difference(innerVars, ambientParams.keySet());
   ...
   * items.
   */
  private SubstMap finishDef(final TypeDef def, final Set<TypeVar> ambientVars,
      final SubstMap ambientParams)
  {
    ...
    if (quantDeclType.hasVars())
    {
      ...
      if (Session.isDebug())
        Session.debug(def.getLoc(), "{0} {1}",
            ambientParams.isEmpty() ? "deferring" : "re-deferring", def.dump());
    }
    else
    {
      if (Session.isDebug())
  ...
  public boolean unify(final Loc loc, final Type t1, final Type t2)
  {
    ...
    if (newSubs != null)
    {
      if (Session.isDebug())
        Session
            .debug(loc, "unify {0}, {1}: mgu {2}", t1sub.dump(), t2sub.dump(),
                newSubs.dump());
    ...
    {
      if (Session.isDebug())
        Session.debug(loc, "unify {0}, {1}: FAILS, subs {2}", t1sub.dump(),
            t2sub.dump(), subs.dump());
  ...
  public Type errorFormat(final Type type)
  {
    ...
    final SubstMap paramMap =
        subsType.buildParamMap(subsType.getVars(), 0, this);
   ...
   */
  @Override protected Type visitBinding(final Binding binding)
  {
    if (Session.isDebug())
    ...
    if (Session.isDebug())
      Session
          .debug(binding.getLoc(), "< {0} : {1}", binding.dump(), type.dump());
   ...
   */
  @Override public Type visit(final LetBinding let)
  {
    if (let.isIntrinsic())
    {
      return let.getType();
    ...
    if (!unify(let.getLoc(), letType, rhsType))
      Session
          .error(let.getLoc(), "let type {0} is incompatible with rhs type {1}",
              errorFormat(letType).dump(), errorFormat(rhsType).dump());
   ...
   * time.
   */
  @Override public Type visit(final TypeDef typeDef)
  {
    return typeDef;
  }
   ...
   * overridden for debug
   */
  @Override protected Type visitTerm(final Term term)
  {
    if (Session.isDebug())
    ...
    if (Session.isDebug())
      Session.debug(term.getLoc(), "< {0} : {1}", term.dump(), type.dump());
   ...
   */
  @Override public Type visit(final RefTerm ref)
  {
  ...
  @Override public Type visit(final ParamValue paramValue)
  {
    return paramValue.getType();
  }

  @Override public Type visit(final BoolLiteral boolLiteral)
  {
    return boolLiteral.getType();
  }

  @Override public Type visit(final IntLiteral intLiteral)
  {
    return intLiteral.getType();
  }

  @Override public Type visit(final LongLiteral longLiteral)
  {
    return longLiteral.getType();
  }

  @Override public Type visit(final DoubleLiteral doubleLiteral)
  {
    return doubleLiteral.getType();
  }

  @Override public Type visit(final StringLiteral stringLiteral)
  {
    return stringLiteral.getType();
  }

  @Override public Type visit(final SymbolLiteral symbolLiteral)
  {
    return symbolLiteral.getType();
  }

  @Override public Type visit(final ListTerm list)
  {
  ...
  @Override public Type visit(final MapTerm map)
  {
    ...
    for (final Map.Entry<Term, Term> entry : map.getItems().entrySet())
    {
      ...
      if (!unify(key, seedKeyType, keyType))
        Session.error(key.getLoc(),
            ...
            errorFormat(key.getType()).dump(), errorFormat(seedKeyType).dump());
  ...
  @Override public Type visit(final TupleTerm tuple)
  {
  ...
  @Override public Type visit(final RecordTerm rec)
  {
    ...
    {
      if (!key.isConstant())
        Session.error(key.getLoc(), "key {0} is not a compile-time constant",
            key.dump());
  ...
  @Override public Type visit(final VariantTerm var)
  {
    final Loc loc = var.getLoc();

    final Term key = var.getKey();
    final Term keyDeref =
        (key instanceof RefTerm) ? ((RefTerm) key).deref() : key;

    final Type type;
    if (!keyDeref.isConstant())
    {
      Session.error(loc, "variant key {0} is not constant", key.dump());
      type = Types.UNIT;
    }
    else
    {
      ...
      final TypeMap targetMap = new TypeMap(loc, keyEnum, targetMembers);
   ...
   */
  @Override public Type visit(final CondTerm cond)
  {
    assert false : "CondTerm in TypeChecker";
    return Types.UNIT;
  }

  @Override public Type visit(final LambdaTerm lambda)
  {
    ...
    if (Session.isDebug())
      Session.debug(lambda.getLoc(), "seeded {0} : {1}", lambda.dump(),
          lambda.getType().dump());
    ...
    if (!unify(resultStatement.getLoc(), resultType, resultTermType))
      Session.error(resultStatement.getLoc(),
          ...
          errorFormat(resultTermType).dump(), errorFormat(resultType).dump());
   ...
   */
  @Override public Type visit(final ApplyTerm apply)
  {
    ...
    switch (flav)
    {
      case FuncApp:
      {
        ...
        if (!unify(loc, baseType, targetBaseType))
        {
          ...
          Session.error(loc,
              ...
              errorFormat(baseType).dump(), errorFormat(targetBaseType).dump());
        }
      ...
      case StructAddr:
      {
        ...
        final Term argDeref =
            (arg instanceof RefTerm) ? ((RefTerm) arg).deref() : arg;

        if (!argDeref.isConstant())
        {
          Session
              .error(loc, "structure addressing argument {0} is not constant",
                  arg.dump());
        ...
        else if (Types.isRec(baseType))
        {
          ...
          else
          {
            Session.error(loc,
                ...
                errorFormat(keyBaseType).dump(), errorFormat(argType).dump());
        ...
        else if (Types.isTup(baseType))
        {
          ...
          else
          {
            ...
            {
              Session.error(arg.getLoc(), "invalid position {0} for tuple {1}",
                  argDeref.dump(), base.dump());
        ...
        else
        {
          ...
          else
          {
            ...
            final TypeList targetList = new TypeList(loc, targetMembers);
   ...
   *
   */
  @Override public Type visit(final CoerceTerm coerce)
  {
