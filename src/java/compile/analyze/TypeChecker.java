...
public final class TypeChecker extends ModuleVisitor<Type> implements TypeEnv
{
    ...
    public Type visit(final RecordTerm rec)
    {
        ...
        {
            if (!key.isConstant())
                Session.error(key.getLoc(), "key {0} is not a compile-time constant", key.dump());
        ...
        final TypeMap targetMap = new TypeMap(loc, keyEnum, entryTypes);
        final Type type = freshVar(loc, Kinds.STAR, new RecordConstraint(targetMap));
    ...
    public Type visit(final VariantTerm var)
    {
        ...
        {
            Session.error(loc, "key {0} is not a compile-time constant", key.dump());
            type = Types.UNIT;
        }
        else
        {
