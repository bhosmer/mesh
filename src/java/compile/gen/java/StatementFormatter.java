...
public final class StatementFormatter extends BindingVisitorBase<String>
{
    ...
    public void addKeyListConstant(
        final List<SimpleLiteralTerm> keySet,
        final String name)
    {
     ...
     * NOTE: assumes we're generating code for a lambda body, or some fragment that contains no
     * bindings.
     * Not suitable for top-level code containing bindings. For that use
     * {@link #formatTopLevelStatement}.
     */
     ...
     * that value bindings are not declared inline, but are assumed to have been generated
     * elsewhere.
     ...
     */
    public String fixup(
        final Loc loc, final String expr, final Type rtype,
        final Class<?> lclass)
    {
     ...
     */
    public String fixup(
        final Loc loc, final String expr, final Class<?> rclass,
        final Type ltype)
    {
    ...
    {
        final List<SimpleLiteralTerm> keyList = record.getKeyList();
        ...
        final List<String> valueExprs = new ArrayList<String>();
        {
            ...
            {
                final Term value = valueMap.get(key);
                valueExprs.add(formatTermAs(value, Object.class));
            }
        }
        ...
        switch (apply.getFlav())
        {
            ...
            case CollIndex:
            {
                ...
                Session.error(apply.getLoc(),
                    "internal error: invalid base term {0} : {1} in collection index expression " +
                        "{2}",
            ...
            case StructAddr:
            {
                ...
                Session.error(apply.getLoc(),
                    "internal error: invalid base term {0} : {1} in structure address expression " +
                        "{2}",
                    baseTerm.dump(),
                    baseType.dump(),
                    apply.dump()
