...
public final class StatementFormatter extends BindingVisitorBase<String>
{
   ...
   *
   */
  public StatementFormatter(final JavaUnit unit,
  ...
  public void addKeyListConstant(final List<SimpleLiteralTerm> keySet,
      final String name)
  {
  ...
  public String formatInLambdaStatement(final Statement statement)
  {
    ...
    final String result =
        statement.isBinding() ? visitBinding((Binding) statement) :
   ...
   *
   */
  @Override public String visitTerm(final Term term)
  {
    assert false;
    return null;
  }
   ...
   * </ul>
   */
  @Override public String visit(final LetBinding let)
  {
    final Type letType = let.getType();

    final String lhs = lambdaDepth == 0 ? formatNameRef(let) :
        // top-level, LHS assigns predeclared var
    ...
    if (let.isIntrinsic())
    {
      if (Types.isFun(letType))
      {
        ...
        rhs = formatIntrinsicAsRHS(let) + ".setSigDump(\"" +
            StringUtils.escapeJava(TypeDumper.dumpTypeParams(letType)) +
            "\", \"" +
            StringUtils.escapeJava(TypeDumper.dumpWithoutParams(letType)) +
            "\")";
      }
      else
      {
   ...
   *
   */
  @Override public String visit(final ParamBinding param)
  {
   ...
   */
  @Override public String visit(final RefTerm ref)
  {
    ...
    if (bindingScope instanceof Module && bindingScope != currentScope)
    {
      ...
      } else
      {
        ...
        final String moduleNameRef =
            (moduleClassDef != null ? moduleClassDef.getName() :
   ...
   */
  @Override public String visit(final BoolLiteral boolLiteral)
  {
   ...
   */
  @Override public String visit(final IntLiteral intLiteral)
  {
   ...
   */
  @Override public String visit(final DoubleLiteral doubleLiteral)
  {
   ...
   */
  @Override public String visit(final StringLiteral stringLiteral)
  {
    final String expr =
   ...
   */
  @Override public String visit(final SymbolLiteral symbolLiteral)
  {
    ...
    final String expr =
        formatType(Types.SYMBOL) + ".get(\"" + symbolLiteral.getValue() + "\")";
   ...
   */
  @Override public String visit(final ListTerm list)
  {
   ...
   */
  @Override public String visit(final MapTerm map)
  {
   ...
   */
  @Override public String visit(final TupleTerm tuple)
  {
   ...
   */
  @Override public String visit(final RecordTerm record)
  {
    ...
    if (keyListField == null)
      assert false :
          "missing key list field for " + DumpUtils.dumpList(keyList);
    ...
    final String expr =
        Record.class.getName() + ".from(" + keyListField + ", " +
   ...
   */
  public String formatObjectArrayLiteral(
      final Collection<String> formattedItems)
  {
   ...
   * Generate Java source expression denoting a Variant as specified by a literal term
   */
  @Override public String visit(final CondTerm cond)
  {
    ...
    final String expr =
        _cond.class.getName() + "." + Constants.INVOKE + "(" + selExpr + ", " +
            casesExpr + ")";
   ...
   */
  @Override public String visit(final LambdaTerm lambda)
  {
   ...
   */
  @Override public String visit(final ApplyTerm apply)
  {
  ...
  private String formatListApplyTerm(final ApplyTerm apply)
  {
    ...
    final String expr = formatTermAs(base, ListValue.class) + ".get(" +
  ...
  private String formatMapApplyTerm(final ApplyTerm apply)
  {
    ...
    final String expr = formatTermAs(base, MapValue.class) + ".get(" +
  ...
  private String formatRecordApplyTerm(final ApplyTerm apply)
  {
    ...
    {
      Session.error(apply.getLoc(),
          "internal error: arg {0} not found in key list {1}", arg.dump(),
          DumpUtils.dumpList(keyList));

      // name lookup as error fallback
      expr = formatTermAs(base, Record.class) + ".get(" +
          formatTermAs(arg, Object.class) + ")";
    } else
    {
      // positional lookup
  ...
  private String formatPolyRecordApplyTerm(final ApplyTerm apply)
  {
    ...
    final String expr = formatTermAs(base, Record.class) + ".get(" +
        formatTermAs(arg, Object.class) + ")";
  ...
  {
    final String expr =
        formatTermAs(base, Lambda.class) + "." + Constants.APPLY + "(" +
            formatTermAs(arg, Object.class) + ")";
  ...
  public InvokeInfo getInvokeInfo(final Term base)
  {
    if (base instanceof LambdaTerm)
    {
      ...
      final String className =
          unit.ensureLambdaClassName(baseLambda, unitDictionary);

      // can scatter if num args != 1
      final InvokeInfo.InvokeMode mode =
          baseLambda.getParams().size() == 1 ? InvokeInfo.InvokeMode.NoScatter :
        ...
        if (let.isIntrinsic())
        {
          ...
          if (Types.isFun(baseType))
          {
                ...
                Types.tupMembers(paramType) instanceof TypeList)
            {
              ...
              return new InvokeInfo(null, InvokeInfo.InvokeMode.Scatter,
                  lambdaClass);
            } else
            {
              // otherwise, intrinsic takes a single argument
              return new InvokeInfo(null, InvokeInfo.InvokeMode.NoScatter,
                  lambdaClass);
            }
          }
   ...
   *
   */
  @Override public String visit(final CoerceTerm coerce)
  {
